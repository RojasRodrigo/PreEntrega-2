//Objeto tienda virtual
const tiendaVirtual = {
  productos: [],
  ventas: [],

  // Método para agregar un producto a la tienda virtual
  ...
  mostrarProductos: function () {
    if (this.productos.length === 0) {
      alert("No hay productos disponibles en la licoreria virtual.");
      return;
    }

    let mensaje = "Aquí están los productos disponibles:\n\n";
    this.productos.forEach((producto) => {
      mensaje += `${producto.numero} ${producto.nombre} Bs.${producto.precio} (Stock: ${producto.stock})\n`;
    });
    alert(mensaje);
  },
    ...
    while (saldo < totalAPagar) {
      const pago = parseFloat(
        prompt(
          `Inserte Bs${
            totalAPagar - saldo
          } o más si introduce un valor mayor se le devolvera cambio:`
        )
      );
  ...
  realizarVenta: function (producto, cantidad) {
    ...
    if (cambio !== 0) {
      ...
      alert(
        `¡Compra exitosa! Tu cambio es Bs${cambio}. ¡Gracias por tu compra!\nFecha y hora: ${fechaHoraActual}`
      );
  ...
  filtrarProductosPorCategoria: function (conAlcohol) {
    ...
    const productosFiltrados = this.productos.filter((producto) => {
      ...
      if (producto.conAlcohol !== undefined) {
        ...
        alert("Error: Algunos productos no tienen la propiedad 'conAlcohol'.");
        return false;
      }
    });

    if (productosFiltrados.length === 0) {
      alert(`No hay productos ${categoria} disponibles en la tienda virtual.`);
    } else {
...
tiendaVirtual.agregarProducto("Coca-Cola", 1, 13, 10, false);
tiendaVirtual.agregarProducto("Ron Solera", 2, 60.0, 5, true);
tiendaVirtual.agregarProducto("Cerveza Huari", 3, 9.0, 8, true);
tiendaVirtual.agregarProducto("Cerveza Paceña", 4, 8.0, 12, true);
tiendaVirtual.agregarProducto("Hielo", 5, 5, 20, false);
...
alert(`¡Hola ${nombre.trim()}! Bienvenido a tu licoreria virtual.`);
...
while (true) {
  ...
  switch (opcion) {
    case "1":
      ...
      tiendaVirtual.mostrarProductos();
      break;
    ...
    case "2":
      ...
      if (!seleccionCompra) {
        alert("Por favor, ingrese un número o nombre de producto válido.");
        break;
      }

      // Aquí agrego el método de búsqueda pedido para la segunda pre entrega
      const productoSeleccionado = tiendaVirtual.productos.find(
        (producto) =>
      ...
      if (productoSeleccionado) {
        ...
        if (cantidadCompra !== null) {
          // Realiza la venta
          tiendaVirtual.realizarVenta(productoSeleccionado, cantidadCompra);
        }
      } else {
    ...
    case "3":
      ...
      switch (opcionFiltrar) {
        case "1":
          ...
          tiendaVirtual.filtrarProductosPorCategoria(true);
          break;

        case "2":
          // Filtra productos sin alcohol
          tiendaVirtual.filtrarProductosPorCategoria(false);
          break;
